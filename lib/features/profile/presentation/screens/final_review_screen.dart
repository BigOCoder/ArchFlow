...
import 'package:archflow/features/auth/presentation/providers/profile_provider.dart';
...
class _FinalReviewScreenState extends ConsumerState<FinalReviewScreen> {
  ...
  Future<void> _submitProfile() async {
    ...
    // Build profile request from onboarding state
    final profile = ProfileRequestModel(
      education: s.educationLevel?.backendValue ?? 'UNDERGRADUATE',
      ...
      databaseKnowledge: DatabaseKnowledgeModel(
        ...
        comfortLevel: s.databaseComfortLevel?.backendValue ?? 'BASIC_QUERIES',
        databasesKnown: s.databasesUsed,
      ),
      codingFrequency: s.codingFrequency?.backendValue ?? 'SOMETIMES',
      debuggingConfidence: s.debuggingConfidence?.backendValue ?? 'MEDIUM',
      problemSolvingArea: s.problemSolvingAreas.isNotEmpty
          ? s.problemSolvingAreas.first
          : 'DSA_BASICS',
    );
